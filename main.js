...
function cancelarReserva() {
    const tiempoRestante = document.querySelector('.top-corn h1');
    
    // Confirmar si el usuario desea cancelar la reserva
    if (confirm("¿Estás seguro de que quieres cancelar la reserva?")) {
        // Cambiar el tiempo restante a "0 horas"
        tiempoRestante.innerHTML = 'Tiempo restante: 0 horas';
        alert("Tu reserva ha sido cancelada.");

        // Preguntar si el usuario desea cambiar la reserva en lugar de cancelarla
        if (confirm("¿Quieres cambiar la reserva en lugar de cancelarla?")) {
            // Redirigir al usuario a la página para cambiar la reserva
            window.location.href = 'aulas-profe.html';
        }
    }
}

// Función para alargar la reserva
function alargarReserva() {
    const tiempoRestante = document.querySelector('.top-corn h1');

    // Solicitar al usuario el tiempo adicional en horas
    let tiempoAdicional = prompt("¿Cuántas horas quieres añadir a la reserva?");
    
    // Validar que el usuario ingrese un número válido
    if (tiempoAdicional && !isNaN(tiempoAdicional) && tiempoAdicional > 0) {
        // Actualizar el tiempo restante en la interfaz
        tiempoRestante.innerHTML = `Tiempo restante: ${tiempoAdicional} horas`;
        alert(`Tu reserva ha sido alargada por ${tiempoAdicional} horas.`);
    } else {
...
document.addEventListener('DOMContentLoaded', function() {
    // Botón "Alargar reserva"
    const btnAlargar = document.querySelector('.cent .can:nth-child(1) a.hola'); // Primer enlace con clase "hola"
    btnAlargar.addEventListener('click', function(event) {
        event.preventDefault(); // Prevenir el comportamiento por defecto del enlace
        alargarReserva();
    });

    // Botón "Cancelar reserva"
    const btnCancelar = document.querySelector('.cent .can:nth-child(2) a.hola'); // Segundo enlace con clase "hola"
